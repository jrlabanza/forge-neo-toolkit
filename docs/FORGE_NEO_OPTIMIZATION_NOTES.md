...
| `sd-forge-guide` | **Guide** tab | The studio manual: every tool, its workflow, maintenance scripts, rollbacks — plus these full notes embedded. |
| Gallery power-ups | inside **Gallery** | 🧺 selection set (bulk favorite / bulk trash-to-`output/_trash`), ⚖ A/B compare, → Director handoff. Tag autocomplete in every prompt field studio-wide (`elem_classes=["prompt"]`). |
| Director macros | inside **Director** | 📥 From Gallery receiver + 🏁 Finalize (enhance → optional cutout → `output/final/`). |
| NAI Director parity complete | inside **Director** | ✒ Line art + ✏ Sketch (live preprocessor via /controlnet/detect, self-selects the best installed module) + 🖍 Colorize (img2img with the ControlNet-union promax holding the lines; graceful fallback without it). First use of a preprocessor downloads its annotator model. |
| Civitai downloader | inside **Civitai** | Search LORAs/checkpoints, one-click download to the right folder, trigger words + preview auto-written. Private-model auth via `api_key` in civitai_settings.json. |
